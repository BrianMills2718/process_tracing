...
| C-005 | open | 1 | The assistant emits a draft source-packet artifact, but the pipeline does not yet consume a source-packet contract. | Without Slice 1, source-packet drafting can still remain adjacent to inference rather than governing it. | Slice 1 must define the accepted packet contract and decide how report audit distinguishes corpus caps from model/report failures. |
