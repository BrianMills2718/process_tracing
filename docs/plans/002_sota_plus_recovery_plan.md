...
|---|---|---|---|
| Source packet / source design | Implemented contract and Brumaire benchmark packet; pipeline/report/audit consume source-packet metadata, and acquisition planning now exists, but the full source-design loop is not yet closed. | Agent builds and iteratively critiques a source design with provenance, dates, genre, reliability, observability, coverage map, and ranked acquisition/disposition actions, then assembles/verifies the corpus from that design. | Source packet schema + one benchmark packet + acquisition agenda are implemented; next gate is iterative source-design expansion, packet-source coverage verification, and explicit disposition of remaining high-value gaps. |
