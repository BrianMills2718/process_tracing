...
from pt.multi_pipeline import _apply_confidence_threshold, _run_single_case
from pt.pass_binarize import _BinarizationResponse
...
def _minimal_result():
    ...
    from pt.schemas import (
        AbsenceResult, Evidence, ExtractionResult, Hypothesis, HypothesisSpace,
        HypothesisTestResult, HypothesisVerdict, ProcessTracingResult,
    ...
    testing = TestingResult(hypothesis_tests=[HypothesisTestResult(
        hypothesis_id="h1", prediction_classifications=[], evidence_evaluations=[])])
    ...
    return ProcessTracingResult(extraction=ext, hypothesis_space=hs, testing=testing,
        absence=AbsenceResult(), bayesian=run_bayesian_update(testing), synthesis=synth)
